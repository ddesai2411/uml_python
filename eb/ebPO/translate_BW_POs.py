...
def get_currLead(ebProjs,currFMP):
    #240209 We may want this earlier so the byte version is available everywhere
    bCurrFMP = bytes(currFMP, encoding="utf-8")
    #print("240209...@@@ got to get_currLead", ebProjs[bCurrFMP])
    try:
        retVal = ebProjs[bCurrFMP]["FMlead"]
...
def parse_POcsv(theCSV,currStamp):
    ...
    with open(theCSV,encoding=encodingFormat) as csvfile:
        ...
        for r in POdata:
            # currFMP can be blank, first line?
            print("\n\n240106 start of POdata loop::", myCount, r["PO #"], samePO)
            ...
            if currPO == r["PO #"]:
                ...
                if currFMP == "NON FMP":
                    ...
                    currFMlead = get_currLead(ebProjs, currFMP)
                    print("240209 here@@@@@@@@" ,currFMP, currFMlead)
                    print("Excel:", costWS, firstCost)
                    wPOC.write_PO_Cost_line(r,ebCompanies,currFundRule, currST, budgTasks, costWS, samePO, currFMlead)
                ...
                if currPO in activePOs:
                    ...
                    if currFMP == "NON FMP":
                        ...
                        # 240209: Need to get lead from Project info.
                        #currLead = "TBD"
                        currLead = currFMlead
                        print("240210 GOT HERE" , currFMP, lineType, currFMlead)                    
                        print(costWS, samePO, currFMlead)
