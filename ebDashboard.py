...
style.configure('Warning.TLabel', font=('Arial', 8, 'italic', ), foreground = 'red', align ='left')
style.configure('TButton', font=('Arial', 9, 'bold'), width=dWidth)
style.configure('TLabel', font=('Arial', 8), foreground = 'blue', )
style.configure('TLabelframe.Label', font=('Consolas', 12, 'bold', 'underline'), foreground = 'black')


# E-Builder Data Import
frame_ebData = ttk.LabelFrame(window,text="E-Builder JSON Data Import",)
frame_ebData.grid(column=1, row=1, padx=xdPad, pady=fypad*5, sticky=tk.W)
...
frame_bw2eb = ttk.LabelFrame(window,text="Buyways to E-Builder Import",)
frame_bw2eb.grid(column=1, row=4, padx=xdPad, pady=fypad*5, sticky=tk.W)

btn_csv = ttk.Button(frame_bw2eb, text="Browse Buyways CSV input file", command=browseCSV, width=dWidth)
btn_csv.grid(column=1, row=5, padx=xdPad, pady=ydPad, sticky=tk.W)

label_csv = ttk.Label(frame_bw2eb, text="No file selected")
label_csv.grid(column=1, row=6, padx=xdPad, pady=ydPad, sticky=tk.W)

btn_run = ttk.Button(frame_bw2eb, text="Run bw2eb", command=run_bw2eb, width=dWidth)
btn_run.grid(column=1, row=7, padx=xdPad, pady=ydPad, sticky=tk.W)

lblResults = ttk.Label(frame_bw2eb, text="Buyways File import Results:\n...", anchor="w")
lblResults.grid(column=1, row=8, padx=xdPad, pady=(2 * ydPad), sticky=tk.W)

btn_bwebJoined = ttk.Button(frame_bw2eb, text="Run bwebJoined", command=run_bw2ebJoined, width=dWidth)
btn_bwebJoined.grid(column=1, row=9, padx=xdPad, pady=ydPad, sticky=tk.W)

lbl_bwebJoinedresults = ttk.Label(frame_bw2eb, text="Buways Files Import Results:\n...\n...\n...", anchor="w")
lbl_bwebJoinedresults.grid(column=1, row=10, padx=xdPad, pady=(2 * ydPad), sticky=tk.W)

btn_bwebXML = ttk.Button(frame_bw2eb, text="Run bweb XML", command=run_bw2ebXML, width=dWidth)
btn_bwebXML.grid(column=1, row=11, padx=xdPad, pady=ydPad, sticky=tk.W)

lbl_bwebXMLresults = ttk.Label(frame_bw2eb, text="Buways Files Import Results:\n...\n...\n...", anchor="w")
lbl_bwebXMLresults.grid(column=1, row=12, padx=xdPad, pady=(2 * ydPad), sticky=tk.W)

# Time Allocation
frame_timeAlloc = ttk.LabelFrame(window, text="Time Alocation Reports")
frame_timeAlloc.grid(column=1, row=13, padx=xdPad, pady=fypad, sticky=tk.W)

btn_TAcsv = ttk.Button(frame_timeAlloc, text="Browse for Speedtype, PS Project Report CSV", command=browseTAcsv, width=dWidth)
btn_TAcsv.grid(column=1, row=14, padx=xdPad, pady=ydPad, sticky=tk.W)

label_TAcsv = ttk.Label(frame_timeAlloc, text="No file selected")
label_TAcsv.grid(column=1, row=15, padx=xdPad, sticky=tk.W)

btn_timeAlloc = ttk.Button(frame_timeAlloc, text="Run timeAlloc", command=run_timeAlloc, width=dWidth)
btn_timeAlloc.grid(column=1, row=16, padx=xdPad, pady=ydPad, sticky=tk.W)

lbl_timeAllocResults = ttk.Label(frame_timeAlloc, text="Time Allocation Results:\n...", anchor="w")
lbl_timeAllocResults.grid(column=1, row=17, padx=xdPad, pady=(2 * ydPad), sticky=tk.W)

# ebFMP
frame_FMP = ttk.LabelFrame(window, text="FMP Reports")
frame_FMP.grid(column=1, row=18, padx=xdPad, pady=fypad, sticky=tk.W)

btn_ebFMP = ttk.Button(frame_FMP, text="Run ebFMP", command=run_ebFMP, width=dWidth)
btn_ebFMP.grid(column=1, row=19, padx=xdPad, pady=ydPad, sticky=tk.W)

lbl_ebFMPresults = ttk.Label(frame_FMP, text="eb FMP Results:\n...", anchor="w")
lbl_ebFMPresults.grid(column=1, row=20, padx=xdPad, pady=(2 * ydPad), sticky=tk.W)

# Monthly Status
frame_moStat = ttk.LabelFrame(window, text="Monthly Status Reports")
frame_moStat.grid(column=1, row=21, padx=xdPad, pady=fypad, sticky=tk.W)

btn_monthStat = ttk.Button(frame_moStat, text="Run for Monthly Status", command=run_monthStat, width=dWidth)
btn_monthStat.grid(column=1, row=22, padx=xdPad, pady=ydPad, sticky=tk.W)

lbl_moStatResults = ttk.Label(frame_moStat, text="Monthly Status Report:\n...", anchor="w")
lbl_moStatResults.grid(column=1, row=23, padx=xdPad, pady=(2 * ydPad), sticky=tk.W)

# eb2bw POREQ
frame_eb2bw = ttk.LabelFrame(window, text="E-Builder to Buyways")
frame_eb2bw.grid(column=1, row=24, padx=xdPad, pady=fypad, sticky=tk.W)

btn_eb2bw = ttk.Button(frame_eb2bw, text="Run eb2bw",command=run_eb2bw,width=dWidth)
btn_eb2bw.grid(column=1,row=25, padx=xdPad, pady=ydPad,sticky=W)
lbl_eb2bw_results = ttk.Label(frame_eb2bw, text="eb2bw Results:\n...\n...\n...", anchor="w")
lbl_eb2bw_results.grid(column=1,row=26,padx=xdPad, pady=(2*ydPad),sticky=W)
warning = ttk.Label(frame_eb2bw, text="WARNING!!! eb2bw Not completely developed in New API", style='Warning.TLabel')
warning.grid(column=1, row=27, padx=xdPad, pady=ydPad, sticky=tk.W)
